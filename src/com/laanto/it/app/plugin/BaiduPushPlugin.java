...
import com.laanto.it.app.util.BaiduPushUtils;
            ...
            synchronized (cbLock) {
                try {
                    if ("startWork".equals(action)) {
                        ...
                        LogManager.i(TAG,apiKey); 
                        PushManager.startWork(cordova.getActivity().getApplicationContext(), PushConstants.LOGIN_TYPE_API_KEY, BaiduPushUtils.getMetaValue(cordova.getActivity(), apiKey));
                    ...
                    } else if("messageGet".equals(action)){
                    	String  data  = this.cordova.getActivity().getIntent().getStringExtra(ApplicationConstants.PATH_INTENT_DATA);
